...
<div class="container">
    ...
    <div style="text-align: right;">
        <label><a class="btn btn-success" href="{{route('univer.studentsToExcel')}}">Экспорт <i class="fas fa-file-export"></i></a></label>
        <button onclick="showImportView()" class="btn btn-primary">Импорт <i class="fas fa-file-import"></i></button>
        <div id="importForm" style="display: none;">
            <form action="{{route('univer.importStudents')}}" enctype="multipart/form-data" method="POST">
                @csrf
                <a href="{{asset('import/шаблон.xlsx')}}"><i class="far fa-file-alt"></i> Скачать пример</a> 
            ...
            <table style="background-color: #fff;" class="table table-striped table-hover table-bordered">
                ...
                @foreach($students as $student)
                    ...
                    <tr>
                        ...
                        <td>{{$student->getDetails()->speciality}}</td>
                        <td>
                            @if($student->isResumeExist())
                                ...
                                <a target="_blank" href="{{route('univer.editResume', ['id'=>$student->id])}}" class="btn btn-warning"><i class="fas fa-pen"></i></a>
                            @else
                                <button disabled class="btn btn-primary"><i class="far fa-file-excel"></i></button>
                                <a target="_blank" href="{{route('univer.editResume', ['id'=>$student->id])}}" class="btn btn-success"><i class="fas fa-plus"></i></a>
                            @endif
                        </td>
                        <td>
                            <a href="{{route('univer.editProfile', ['id'=>$student->id])}}" class="btn btn-warning"><i class="fas fa-pen"></i></a>
                            ...
                            <div class="modal-dialog modal-dialog-centered" role="document">
                                ...
                                <div class="modal-footer">
                                    ...
                                    <a href="{{route('univer.removeStudent', ['id'=>$student->id])}}"><button type="button" class="btn btn-danger">Удалить</button></a>
                                </div>
                                </div>
                            </div>
                            </div>
